...
{
    private Dictionary<GoalkeeperState, Vector3> _spriteStates;
    ...
    private void UpdateGoalkeeperPosition()
    {
        ...
        var x = _leftZone.IsBeingLookedAt ? -1.0f : _centerZone.IsBeingLookedAt ? 0.0f : _rightZone.IsBeingLookedAt ? 1.0f : 0.0f;
        var targetPosition = new Vector3(_position.x + x, _position.y, _position.z);

        // Lerp towards the target position (1 unit/sec).
        var speed = Time.deltaTime * 1.0f;
